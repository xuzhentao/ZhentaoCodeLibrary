...
class Class_MachineLearning():

	# constructor.	
	def __init__(self, whether_plot = True, whether_mute = False):
		self.X_train = None
		self.y_train = None
		self.X_test = None
		self.Y_test = None
	...
	def classification_explore(self):
		...
		clf = GradientBoostingClassifier(n_estimators=conf["n_estimators"], learning_rate=conf["learning_rate"], max_depth=conf["max_depth"], random_state=conf["random_state"])
		clf.fit(train_X, train_Y)
		test_pred = clf.predict(test_X)
		classifier = "GBC"
		accuracy = accuracy_score(test_Y, test_pred)
		...
		clf = MLPClassifier(solver=conf['solver'],hidden_layer_sizes=conf['hidden_layer_sizes'], max_iter=conf['max_iter'])
		clf.fit(train_X, train_Y)
		test_pred = clf.predict(test_X)
		classifier = "MLP"
		accuracy = accuracy_score(test_Y, test_pred)
		...
		if self.whether_plot:
			...
			list_accuracyscore_sort, list_classifierName_sort = zip(*sorted(zip(list_accuracyscore, list_classifierName),reverse = False))
		...
		self.classification_result = {classifiername: accuracyscore for classifiername, accuracyscore in zip(list_classifierName, list_accuracyscore)}
