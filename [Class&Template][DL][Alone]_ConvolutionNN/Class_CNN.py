		...
		with tf.Session() as sess:
			...
			for epoch in range(self.total_epoches):
				...
				for i in range(int(self.X_train.shape[0] / self.batch_size)):
					...
					#train & result 
					_, pred_value_train , cost_value_train = sess.run([optimizer, pred,cost], feed_dict={x:batch_x_train, y:batch_y_train})
					train_accuracy = np.sum([ int(i == j) for i,j in zip(np.argmax(batch_y_train, axis = 1), np.argmax(pred_value_train, axis = 1))]) / pred_value_train.shape[0]
				...
				test_accuracy = np.sum([ int(i == j) for i,j in zip(np.argmax(batch_y_test, axis = 1), np.argmax(pred_value_test, axis = 1))]) / pred_value_test.shape[0]
				...
				#print the result.
				print(str(epoch) + '/' + str(self.total_epoches) + "\t[Loss(train),\tLoss(test),\tAccu(train),\tAccu(test)] = [" + str(avg_train_loss) + '\t' + str(cost_value_test) + '\t' + str(avg_train_accu) + '\t' +  str(test_accuracy) + ']' )
